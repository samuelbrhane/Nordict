...
import { TERMS_SECTIONS } from "@/config/terms";
...
const TermsContent = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sectionElements = TERMS_SECTIONS.map((s) =>
        ...
        <div className="grid gap-8 lg:grid-cols-[250px_1fr] lg:gap-12">
          ...
          >
            <details className="group rounded-xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
              ...
              <nav className="border-t border-neutral-200 px-2 py-2 dark:border-neutral-800">
                {TERMS_SECTIONS.map((section) => (
                  <button
                    key={section.id}
                    onClick={() => scrollToSection(section.id)}
          ...
          >
            ...
            >
              <p className="text-[10px] text-neutral-500 sm:text-xs dark:text-neutral-400">
                By using Nordict, you acknowledge that you have read,
